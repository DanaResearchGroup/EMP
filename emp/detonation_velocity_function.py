...
checkpoint_path = Path(__file__).resolve().parent.parent / "models/best_velocity_training_2.0.ckpt"
mpnn = models.MPNN.load_from_checkpoint(checkpoint_path)

def predict_detonation_velocity(molecule):
    """
    Function to predict the value of detonation velocity for a given molecule
    Argument:
        molecule (Union[str, rdkit.Chem.rdchem.Mol]): Either a SMILES string or an RDKit Molecule object.
    Returns:
        detonation velocity prediction (float): Predicted value by the trained NN.
    ...
    try:
        ...
        detonation_velocity_pred = test_preds[0][0]
        return detonation_velocity_pred
